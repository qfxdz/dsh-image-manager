...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let sessionsService;
		/** 客户端工作区服务（ctx.workspaces）：归档会话集合来自它的 list.archivedSessionIds。 */
		let workspacesService;
		/** 常量空数组，避免每次渲染都新建引用导致 hook 抖动。 */
		 ...
		 */
		function buildSessionRows(catalog, live, archivedSessionIds) {
			const archived = new Set(archivedSessionIds ?? []);
			const liveById = new Map((live ?? []).map((item) => [item.id, item]));
			let rows;
			if (catalog?.ids === undefined) {
				// 目录不可用：退回 Host live 列表，至少按归档集合过滤一次。
				rows = (live ?? [])
					.filter((item) => !archived.has(item.id))
					.map((item) => ({ ...item, title: item.title ?? item.id, live: true, updatedAt: 0, blank: false }));
			} else {
				rows = catalog.ids
					.map((id) => {
						const summary = catalog.byId?.[id];
						if (summary === undefined) return undefined;
						// 与 dsh 自己的会话栏同口径（dsh-client-ui-workspace 的 sessionVisible）：
						// 子代理会话、归档会话都不出现；空会话(blank)只出现「当前」那一个，
						// 否则每个工作区都会多出一条以文件夹名命名的假会话。
						if (summary.origin === 'subagent') return undefined;
						if (archived.has(id)) return undefined;
						if (summary.blank === true && id !== catalog.current) return undefined;
			...
			// 同名会话（未命名会话的标题会退化成项目文件夹名）补一个短 id，避免看起来像重复项。
			const counts = new Map();
			for (const row of rows) counts.set(row.title, (counts.get(row.title) ?? 0) + 1);
			for (const row of rows) {
				if ((counts.get(row.title) ?? 0) > 1) row.title = `${row.title} · ${String(row.id).slice(-6)}`;
			}
			return rows;
		}

		/** 订阅一个 ObservableSnapshot（优先用 useSyncExternalStore，旧 React 退回 useState）。 */
		function useSnapshot(subscribe, getSnapshot) {
		...
		/** 订阅客户端会话目录（SessionListState）。服务不可用时返回 null。 */
		function useCatalog() {
			...
			return useSnapshot(subscribe, getSnapshot);
		}

		/** 订阅工作区状态，取出「全局归档会话集合」。 */
		function useArchivedIds() {
			const source = workspacesService?.list;
			const subscribe = React.useCallback(
				(notify) => (typeof source?.subscribe === 'function' ? source.subscribe(notify) : () => {}),
				[source],
			);
			const getSnapshot = React.useCallback(() => source?.getSnapshot?.() ?? null, [source]);
			const snapshot = useSnapshot(subscribe, getSnapshot);
			return snapshot?.archivedSessionIds ?? EMPTY_ARCHIVED;
		}
		...
		function ImageManager({ mode = 'session', sessionId: initialSessionId, sessions, reloadSessions }) {
			...
			const rows = React.useMemo(() => buildSessionRows(catalog, sessions ?? [], archived), [catalog, sessions, archived]);
		...
		function apply(ctx) {
			...
			try {
				ctx.inject?.(['workspaces'], (scoped) => {
					workspacesService = scoped.workspaces ?? workspacesService;
				});
			} catch {
				/* 同上，降级为「不排除归档会话」 */
			}
