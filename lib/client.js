	...
	factory: (require) => {
		...
		function ImageManager({ mode = 'session', sessionId: initialSessionId, sessions, reloadSessions }) {
			...
			return h(
						...
						[
							...
							...(sessions ?? []).map((session) =>
								h(
									...
									`${session.title ?? session.id}（${session.images} 图 · ${session.inherit ? `跟随全局 ${session.maxImages}` : `自定义 ${session.maxImages}`}）`,
								),
							),
						],
					),
				...
				state === null
					? h('div', { style: styles.muted }, '选择一个会话后即可管理它的图片。')
					: h(
							'div',
							{ style: styles.muted },
		...
		function SettingsSection() {
			...
			return h(
				...
				h(
					...
					sessions.length === 0
						? '本进程还没有已加载的会话。'
						: `已加载会话：${sessions.map((session) => `${session.title ?? session.id}（${session.images} 图${session.inherit ? '' : '·自定义'}）`).join('、')}`,
				),
			);
		}
